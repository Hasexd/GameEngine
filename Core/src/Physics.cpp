...
namespace Core
{
	...
	RaycastHit PhysicsWorld::Raycast(const glm::vec3& origin, const glm::vec3& direction, float maxDistance)
	{
		...
		for (const auto& obj : m_RegisteredObjects)
		{
			...
			RaycastHit hit = TestRayAABB(origin, direction, obj, maxDistance);
	...
	RaycastHit PhysicsWorld::TestRayAABB(const glm::vec3& origin, const glm::vec3& direction, const std::shared_ptr<Object>& object, float maxDistance)
	{
		RaycastHit hit;
		...
		glm::vec4 localOrigin = invModel * glm::vec4(origin, 1.0f);
		glm::vec4 localDirection = invModel * glm::vec4(direction, 0.0f);
	...
	bool PhysicsWorld::RayAABBIntersection(const glm::vec3& origin, const glm::vec3& direction, const glm::vec3& boxMin, const glm::vec3& boxMax, float& distance)
	{
